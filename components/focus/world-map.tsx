...
// Diğer kullanıcı uçaklarının renkleri (sırayla)
...
export function WorldMap({ departure, destination, progress, otherFlights = [] }: WorldMapProps) {
  ...
  // Diğer kullanıcı markerları: username → { marker, innerEl }
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      innerEl.style.cssText = `width:72px;height:72px;background:transparent;transform:rotate(${bearing + 45}deg);transition:transform 0.4s ease;filter:drop-shadow(0 2px 8px rgba(0,0,0,0.7)) drop-shadow(0 0 4px rgba(0,0,0,0.5));`;
  ...
  // ── Diğer kullanıcıları güncelle / ekle / kaldır ─────────────────────────
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !map.loaded()) return;

    const currentUsernames = new Set(otherFlights.map((f) => f.username));
    ...
    // Her aktif kullanıcı için marker ekle veya güncelle
    otherFlights.forEach((flight, i) => {
      ...
      if (existing) {
        ...
        label.style.cssText = `background:${color};color:white;font-size:10px;font-weight:700;padding:2px 6px;border-radius:10px;margin-bottom:2px;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.4);font-family:system-ui,sans-serif;`;
        label.textContent = flight.username;

        // Uçak ikonu (renkli daire)
        const inner = document.createElement("div");
        inner.style.cssText = `width:36px;height:36px;background:transparent;transform:rotate(${brng + 45}deg);transition:transform 0.4s ease;filter:drop-shadow(0 0 6px ${color});`;
